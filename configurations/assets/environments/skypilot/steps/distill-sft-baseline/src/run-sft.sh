...
#   path   steps/distill-sft-baseline/src/run-sft.sh
...
#
# distill-sft-baseline step entrypoint, INSIDE the container.
#
...
if [[ ! -f "$STEP_HOME/render_sft_config.py" ]]; then
  ...
  echo "       In the image it is COPYd there from steps/distill-sft-baseline/src/; in a" >&2
