# GOLD fully-on-policy smoke recipe

**Fully** on-policy GOLD distillation (`lmbda 1.0`) with vLLM as a **separate target**
...
already proved end to end, at the fully-generated end of the loss spectrum.
...
- `gold-blend-smoke` (this family) is the same shape and pair at `lmbda 0.3`.
- **This recipe** is the `lmbda 1.0` endpoint: every sample the trainer sees is generated
  by the student under vLLM, none drawn from the fixed corpus. Not yet run.

## Why `lmbda` above 0 changes the topology, not just the loss

The trainer needs a vLLM server to generate from whenever `lmbda > 0` — `vllm_num_servers`
...
generates), but **not** correct for a real on-policy run. Per `distill-gold`'s own
`USAGE.md` and `gold-onpolicy-smoke`'s README: on-policy should continue from a good
off-policy checkpoint, not the base student, since a base student generates noise under
vLLM.
...
2. Pass that path into this recipe:
   ```
   gb build start -f recipes/granite4-gold-distillation/lsf/gold-onpolicy-full-smoke/build.yaml \
...
```
gb build start -f recipes/granite4-gold-distillation/lsf/gold-onpolicy-full-smoke/build.yaml --space <your-space>
```
...
on-policy path is genuinely exercised, not just configured.
