...
#   path   steps/distill-sft-baseline/src/render_sft_config.py
...
#
"""Validate distill-sft-baseline's step config and render it as an SFT YAML file.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"FATAL [distill-sft-baseline config]: {exc}", file=sys.stderr)
        return 2
    out = Path(args.out)
